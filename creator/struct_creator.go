...
func NewStruct(pkg, name string) *StructCreator {
	if pkg == "" || name == "" {
		panic("pkg or name can not be empty")
	}
...
// AddImports for struct
func (s *StructCreator) AddImports(imps ...string) *StructCreator {
	s.Imports = append(s.Imports, imps...)
	return s
}
...
func (s *StructCreator) Exec() error {
	...
	if s.output == nil {
		...
		if os.IsNotExist(err) {
			...
			output, err := os.OpenFile(path, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0666)
			if err != nil {
				return err
			}
			s.SetOutput(output, true)
